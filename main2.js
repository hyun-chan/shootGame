...
function setupkeyboardListener() {
  ...
  document.addEventListener("keyup", function (event) {
    ...
    if (event.key == "spacebar" || event.keyCode == 32) {
...
function loadImage() {
  ...
  gameOverImage.style.opacity = 0.1;
}
...
function render() {
  ...
  for (let i = 0; i < bulletList.length; i++) {
    if (bulletList[i].alive) {
      ctx.drawImage(bulletImage, bulletList[i].x, bulletList[i].y, 30, 30);
    }
  }
